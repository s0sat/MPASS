...
if ($help == 1) {
	...
	print "\t\-v Version\n\n\n";
	print "Requirements\:\n";
	print "\tProtein FASTA files \(created by metagenome assembly and gene prediction\)\n";
	print "\t\tFilenames should end with \"_aa\"\. No file extension is required \(E\.g\. Filename_aa\)\.\n\n";
...
foreach $query (@qlist) {
	...
	if (length($query2) < 8) {
		...
		$query2 = substr($query2, 0, 8); # Initial 8 characters from longer name are used
	}
	print OUT "$query2  "; 
